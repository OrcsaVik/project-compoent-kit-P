...
package com.github.framework.designpattern.strategy;

import com.github.framework.base.ApplicationContextHolder;
import com.github.framework.base.listen.ApplicationInitializingEvent;
import com.github.framework.convention.exception.ServiceException;
...
public class AbstractStrategyChoose implements ApplicationListener<ApplicationInitializingEvent> {
     ...
     *
     * @param mark          策略标识
     * @param predicateFlag 匹配范解析标识 正则表达式
     * @return 实际执行策略‘// 假设订单标记为"SECKILL_1001"
     *
     * String orderMark = "SECKILL_1001";
     * // 获取匹配的策略（会匹配到SeckillOrderStrategy）
     * OrderExecuteStrategy strategy = orderStrategySelector.selectStrategy(orderMark);
     */
    public AbstractExecuteStrategy choose(String mark, Boolean predicateFlag) {
        if (predicateFlag != null && predicateFlag) {
            return abstractExecuteStrategyMap.values().stream()
                    ...
                    // "SECKILL_\\d+"; // 匹配以SECKILL_开头的标记 设置对应匹配符
